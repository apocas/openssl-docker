...
Openssl.prototype.csr = function (domain, options, callback) {
  ...
  if (!options.keyType) options.keyType = 'rsa:2048';
  ...
  if (options.read === true) {
    cmd += '; chmod 666 ' + containerKeyPath + ' ' + containerCSRPath;
  }
...
Openssl.prototype.pfx = function (domain, options, callback) {
  ...
  if (!options.outputDir) options.outputDir = os.tmpdir();
  if (!options.passin) options.passin = '';
  if (!options.passout) options.passout = '';
  ...
  var opts = [
    ...
    '-passin', 'pass:' + options.passin,
    '-passout', 'pass:' + options.passout
  ];
  ...
  cmd += '; chmod 666 ' + containerPfxPath;
